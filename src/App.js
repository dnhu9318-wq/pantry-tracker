...
const HEADERS = [
  ...
  "Expiry Date", "Calories (kcal)", "Protein (g)", "Carbs (g)",
  "Fat (g)", "Fiber (g)", "Sodium (mg)", "Nutri-Score", "Notes", "Barcode"
];
...
const itemToRow = (item) => [
  ...
  item.unit || "",
  item.expiry || "",
  item.nutrition?.calories != null ? Math.round(item.nutrition.calories) : "",
  item.nutrition?.protein  != null ? item.nutrition.protein.toFixed(1)   : "",
  item.nutrition?.carbs    != null ? item.nutrition.carbs.toFixed(1)     : "",
  item.nutrition?.fat      != null ? item.nutrition.fat.toFixed(1)       : "",
  item.nutrition?.fiber    != null ? item.nutrition.fiber.toFixed(1)     : "",
...
// Load the Google API client + Identity Services scripts
...
const loadGapiScripts = async () => {
  if (window.gapi && window.google?.accounts) return;
  await loadScript("https://apis.google.com/js/api.js");
  await new Promise((resolve, reject) => {
    window.gapi.load("client", { callback: resolve, onerror: reject, timeout: 10000, ontimeout: reject });
  });
...
const fetchNutrition = async (barcode) => {
  try {
    ...
    return {
      ...
      caloriesUnit: n["energy-kcal_serving"] != null ? "per serving" : "per 100g",
      fat:          n["fat_100g"] ?? null,
      ...
      sodium:       n["sodium_100g"] != null ? (n["sodium_100g"] * 1000).toFixed(0) : null,
...
export default function PantryTracker() {
  ...
  const connectGoogle = async () => {
    const isConfigured = GOOGLE_API_KEY !== "YOUR_API_KEY_HERE" &&
                         CLIENT_ID      !== "YOUR_CLIENT_ID_HERE" &&
                         SPREADSHEET_ID !== "YOUR_SPREADSHEET_ID_HERE";
    if (!isConfigured) {
      setSheetMsg("⚠️ Please fill in your API Key, Client ID, and Spreadsheet ID at the top of the code first.");
      setSheetStatus("error");
      return;
    }
    setSheetStatus("signing-in");
    setSheetMsg("Loading Google scripts…");
    try {
      const timeout = new Promise((_, reject) =>
        setTimeout(() => reject(new Error("Timed out. Check your internet connection and try again.")), 15000)
      );
      await Promise.race([loadGapiScripts(), timeout]);
      setSheetMsg("Initializing Sheets API…");
  ...
  const appendToSheet = async (item) => {
    ...
    try {
      await window.gapi.client.sheets.spreadsheets.values.append({
        ...
        resource: { values: [itemToRow(item)] },
      });
  ...
  const updateSheetRow = async (item) => {
    // For simplicity, append an updated row tagged with "(updated)"
    if (sheetStatus !== "ready") return;
    try {
