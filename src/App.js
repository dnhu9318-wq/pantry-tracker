...
const HEADERS = [
  ...
  "Nutri-Score", "NOVA Group", "Allergens", "Notes", "Barcode"
];
...
const fetchNutrition = async (barcode) => {
  try {
    ...
    return {
      ...
      novaGroup:    p.nova_group ? String(p.nova_group) : "",
      ...
      <div onClick={e => e.stopPropagation()} style={{ background:"#fff", borderRadius:"20px 20px 0 0",
        ...
        <div style={{ display:"flex", gap:12, marginBottom:16 }}>
          ...
          <div>
            ...
            <div style={{ display:"flex", gap:8, marginTop:6, flexWrap:"wrap" }}>
              ...
              {nutrition.novaGroup && (() => {
                const nc = { "1":"#22c55e","2":"#84cc16","3":"#f97316","4":"#ef4444" };
                const nl = { "1":"Unprocessed","2":"Processed ingredient","3":"Processed","4":"Ultra-processed" };
                const c = nc[nutrition.novaGroup] || "#94a3b8";
                return (
                  <span style={{ background:c, color:"#fff", borderRadius:20,
                    padding:"2px 10px", fontSize:13, fontWeight:700 }}>
                    NOVA {nutrition.novaGroup} — {nl[nutrition.novaGroup]}
                  </span>
                );
              })()}
            </div>
        ...
        <table style={{ width:"100%", borderCollapse:"collapse", marginBottom:12 }}>
          <tbody>
            ...
            <NRow label="— Added Sugars" val={nutrition.addedSugars} />
...
function ItemForm({ initial, onSave, onCancel, scannedBarcode }) {
  const [form, setForm] = useState(() => initial || {
    ...
    nutrition:null, imageUrl:"", novaGroup:""
  });
  ...
  const lookupBarcode = async (code) => {
    ...
    if (n) {
      ...
      setForm(f => ({ ...f, nutrition:n, name: f.name||n.name||"", brand: f.brand||n.brand||"", imageUrl: f.imageUrl||n.imageUrl||"", novaGroup: f.novaGroup||n.novaGroup||"" }));
    } else {
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.45)", display:"flex",
      ...
      <div style={{ background:"#fff", borderRadius:"20px 20px 0 0", padding:"24px 20px 40px",
        ...
        <div style={{ marginBottom:14 }}>
          <Lbl>Nutrition Facts (manual entry)</Lbl>
          <div style={{ background:"#f8fafc", borderRadius:10, padding:"12px", border:"1.5px solid #e2e8f0" }}>
            <p style={{ margin:"0 0 10px", fontSize:11, color:"#94a3b8", fontWeight:600, textTransform:"uppercase", letterSpacing:"0.05em" }}>
              Leave blank if unknown · values per serving
            </p>
            {[
              { key:"calories",     label:"Calories (kcal)",    step:"1"   },
              { key:"fat",          label:"Total Fat (g)",       step:"0.1" },
              { key:"saturatedFat", label:"Saturated Fat (g)",   step:"0.1" },
              { key:"transFat",     label:"Trans Fat (g)",       step:"0.1" },
              { key:"cholesterol",  label:"Cholesterol (mg)",    step:"1"   },
              { key:"sodium",       label:"Sodium (mg)",         step:"1"   },
              { key:"carbs",        label:"Total Carbs (g)",     step:"0.1" },
              { key:"fiber",        label:"Dietary Fiber (g)",   step:"0.1" },
              { key:"sugars",       label:"Total Sugars (g)",    step:"0.1" },
              { key:"addedSugars",  label:"Added Sugars (g)",    step:"0.1" },
              { key:"protein",      label:"Protein (g)",         step:"0.1" },
            ].map(({ key, label, step }) => (
              <div key={key} style={{ display:"flex", alignItems:"center", justifyContent:"space-between",
                marginBottom:8, gap:12 }}>
                <label style={{ fontSize:13, color:"#475569", flex:1 }}>{label}</label>
                <input
                  ...
                  style={{ width:80, padding:"6px 8px", borderRadius:6, border:"1.5px solid #e2e8f0",
                    fontSize:13, textAlign:"right", background:"#fff", fontFamily:"inherit",
        ...
        <div style={{ marginBottom:14 }}>
          <Lbl>NOVA Group (food processing level)</Lbl>
          <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
            {[
              { val:"1", label:"1 — Unprocessed", desc:"Fruits, veg, meat, eggs", color:"#22c55e" },
              { val:"2", label:"2 — Processed ingredient", desc:"Oil, flour, sugar", color:"#84cc16" },
              { val:"3", label:"3 — Processed", desc:"Canned, cured, fermented", color:"#f97316" },
              { val:"4", label:"4 — Ultra-processed", desc:"Chips, soda, ready meals", color:"#ef4444" },
            ].map(({ val, label, desc, color }) => (
              <button key={val} onClick={() => set("novaGroup", form.novaGroup === val ? "" : val)}
                style={{
                  flex:"1 1 45%", padding:"8px 10px", borderRadius:8, cursor:"pointer",
                  border: form.novaGroup === val ? `2px solid ${color}` : "2px solid #e2e8f0",
                  background: form.novaGroup === val ? color + "18" : "#f8fafc",
                  textAlign:"left"
                }}>
                <div style={{ fontSize:13, fontWeight:700, color: form.novaGroup === val ? color : "#0f172a" }}>{label}</div>
                <div style={{ fontSize:11, color:"#94a3b8", marginTop:2 }}>{desc}</div>
              </button>
            ))}
          </div>
          {form.novaGroup && (
            <button onClick={() => set("novaGroup", "")}
              style={{ marginTop:6, fontSize:11, color:"#94a3b8", background:"none",
...
export default function PantryTracker() {
    ...
    try {
      ...
      tokenClientRef.current = window.google.accounts.oauth2.initTokenClient({
        ...
        callback: async (resp) => {
          if (resp.error) {
            setSheetStatus("error");
            setSheetMsg("Sign-in failed: " + resp.error + " — " + (resp.error_description || ""));
            return;
          }
          if (!resp.access_token) {
            setSheetStatus("error");
            setSheetMsg("No access token received. Please try again.");
            return;
          }
          ...
          try {
            ...
            setSheetStatus("error");
            setSheetMsg("Sheets error: " + e.message + " (Check Sheets API is enabled and spreadsheet ID is correct)");
          }
        },
      });
  ...
  const ensureHeaderRow = async (token) => {
    ...
    const res = await sheetsRequest("GET", `${SPREADSHEET_ID}/values/${SHEET_NAME}!A1:Z1`, null, t);
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  ...
                  <div style={{ display:"flex", alignItems:"center", gap:10, marginTop:8, flexWrap:"wrap" }}>
                    ...
                    {item.novaGroup && (() => {
                      ...
                      const novaLabels = { "1":"NOVA 1","2":"NOVA 2","3":"NOVA 3","4":"NOVA 4" };
                      const c = novaColors[item.novaGroup] || "#94a3b8";
                      return (
                        <span style={{ fontSize:12, fontWeight:700, color:c,
                          background: c+"18", borderRadius:20, padding:"2px 10px" }}>
                          {novaLabels[item.novaGroup] || "NOVA " + item.novaGroup}
                        </span>
                      );
                    })()}
                  </div>
